      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <Link to={token ? "/dashboard" : "/"} className="hover:underline">
              StudyHub
            </Link>
          </motion.div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          {token && userEmail && (
            <div className="hidden items-center gap-3 md:flex">
              <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 px-3 py-1 text-xs font-semibold text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">
              ...
              <button 
                onClick={logout}
                className="rounded-full border border-rose-300 px-3 py-1 text-xs font-semibold text-rose-600 transition hover:bg-rose-50 dark:border-rose-900 dark:text-rose-400 dark:hover:bg-rose-950/30"
              >
                Logout
              </button>
            </div>
          ...
          <button
            onClick={() => setOpen((o) => !o)}
            className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-slate-300 text-slate-800 transition hover:border-indigo-400 dark:border-slate-700 dark:text-slate-100 md:hidden"
            aria-label="Toggle menu"
          >
            {open ? <Cross2Icon className="h-5 w-5" /> : <HamburgerMenuIcon className="h-5 w-5" />}
          </button>
          {!token && (
              ...
              {token ? (
                <>
                  ...
                  <button 
                    ...
                    className="mt-2 rounded-lg bg-rose-500/10 px-3 py-2 text-left font-semibold text-rose-600 dark:bg-rose-500/20 dark:text-rose-400"
                  >
                    Logout
                  </button>
                </>
